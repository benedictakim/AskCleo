...
let rid = null;
let cats = [];
//let colors = ["#ffca5e", "#ffae3e", "#ff8b2a"];
...
let svgs = Array.from(document.querySelectorAll("svg.used"));

svgs.forEach(svg => {
  let tongue = svg.querySelector(".tongue");
  let hex = svg.querySelector(".theHex");
  let p = new Path(tongue);
  cats.push(p);
